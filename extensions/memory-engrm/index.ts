...
class EngramClient {
  ...
  async search(query: string, limit = 5): Promise<EngramMemory[]> {
    try {
    ...
    api.registerTool(
      {
        ...
        async execute(_toolCallId, params) {
          ...
          if (query) {
            ...
            const list = results.map((m) => `- [${m.id.slice(0, 8)}] ${m.text.slice(0, 60)}...`).join("\n");
            return {
      ...
      api.on("before_agent_start", async (event) => {
        ...
        try {
          ...
          if (!context || context.length < 10) {
            // Fallback to search
            const results = await client.search(event.prompt, cfg.recallLimit || 5);
            if (results.length === 0) return;
            
            api.logger.info?.(`memory-engrm: injecting ${results.length} memories`);

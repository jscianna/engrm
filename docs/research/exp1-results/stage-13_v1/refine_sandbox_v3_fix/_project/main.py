...
def calculate_task_completion_rate(condition_name, randomness_factor):
    """
...
def run_experiment(condition_name, seed, harness):
    ...
    np.random.seed(seed)
    try:
        # Introduce a small random factor to simulate variations in task performance
        randomness_factor = np.random.normal(0, 0.05)
        task_completion_rate = calculate_task_completion_rate(condition_name, randomness_factor)

...
def simulate_task_completion_rate(condition_name, seed):
    ...
    This function adds variability and granularity to better reflect realistic outcomes.
    """
    np.random.seed(seed)
    base_rates = {
    ...
    variability = np.random.normal(0, 0.02)  # Add some noise for variability
    task_completion_rate = base_rates.get(condition_name, 0.5) + variability
    
    return max(0.0, min(1.0, task_completion_rate))  # Ensure the rate is within [0, 1]

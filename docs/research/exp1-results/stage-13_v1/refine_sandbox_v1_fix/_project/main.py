...
Model Architecture: Context Injection Model
...
from strategies import context_injection_strategy
...
def run_experiment(condition_name, seed, harness):
    ...
    # Obtain task completion rate from the context injection strategy
    task_completion_rate = context_injection_strategy(condition_name, HYPERPARAMETERS, seed)
    ...
    for condition in conditions:
        try:
            ...
            if metrics:
                ...
                std_metric = np.std(metrics, ddof=1)  # Use ddof=1 for sample standard deviation

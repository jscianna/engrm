...
def simulate_task_completion_rate(condition_name, hyperparams, seed):
    """
    Simulate task completion rate based on the condition, hyperparameters, and seed.
    """
    base_rate = {
    ...
    # Use the seed to create variability in the simulation
    np.random.seed(seed)
    adjustment_factor = 1 + (hyperparams['context_window_size'] / 2048) * 0.05
    noise = np.random.normal(0, 0.02)  # Adding some noise for variability
    task_completion_rate = (base_rate * adjustment_factor) + noise
    return min(max(task_completion_rate, 0.0), 1.0)  # Ensure the rate is between 0 and 1
...
def run_experiment(condition_name, seed, harness):
    ...
    """
    task_completion_rate = simulate_task_completion_rate(condition_name, HYPERPARAMETERS, seed)

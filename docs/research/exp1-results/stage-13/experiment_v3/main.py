...
def run_experiment(condition_name, seed, harness):
    ...
    if condition_name == "StaticContextInjection":
        completion_rate = simulate_task_completion_rate(context_size, 1.0, error_rate)
    elif condition_name == "MultiAgentContextManagement":
        completion_rate = simulate_task_completion_rate(context_size, 1.1, error_rate)
    elif condition_name == "DynamicTaskAwareContextInjection":
        completion_rate = simulate_task_completion_rate(context_size + 200, 1.3, error_rate)
    elif condition_name == "AIRealTimeVulnerabilityDetection":
        completion_rate = simulate_task_completion_rate(context_size, 1.2, error_rate)
    elif condition_name == "NoContextPrioritization":
        completion_rate = simulate_task_completion_rate(context_size, 0.9, error_rate)
    elif condition_name == "HumanOnlyVulnerabilityDetection":
        completion_rate = simulate_task_completion_rate(context_size, 0.7, error_rate)
    else:

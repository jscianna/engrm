...
def simulate_task_completion(condition_name, user_behavior, error_patterns, historical_traces):
    """
    Simulate task completion rate based on condition and input factors.
    """
    base_rate = 0.5  # Base task completion rate

    # Adjust the base rate based on the condition
    ...
    # Simulate the effect of user behavior, error patterns, and historical traces
    user_effect = np.clip(user_behavior * 0.1, -0.1, 0.1)
    error_effect = np.clip(error_patterns * 0.05, -0.05, 0.05)
    historical_effect = np.clip(historical_traces * 0.05, -0.05, 0.05)

    # Calculate final task completion rate
    task_completion_rate = base_rate + user_effect + error_effect + historical_effect
    task_completion_rate = np.clip(task_completion_rate, 0, 1)
...
def run_experiment(condition_name, seed, harness):
    ...
    # Generate random values to simulate user behavior, error patterns, and historical traces
    user_behavior = np.random.normal(loc=0.0, scale=1.0)
    error_patterns = np.random.normal(loc=0.0, scale=1.0)
    historical_traces = np.random.normal(loc=0.0, scale=1.0)

    # Simulate the task completion rate
    task_completion_rate = simulate_task_completion(
        condition_name, user_behavior, error_patterns, historical_traces
    )

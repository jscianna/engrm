...
def run_experiment(condition_name, seed, harness):
    ...
    # Introduce some variability based on random seed
    base_performance = {
    ...
    task_completion_rate = base_performance.get(condition_name, None)
    if task_completion_rate is None:
        print(f"CONDITION_FAILED: {condition_name} unrecognized")
        return None
    
    # Apply a small random variation to simulate realistic conditions
    task_completion_rate += np.random.normal(0, 0.01)

...
def simulate_task_completion(condition_name):
    """
    Simulate task completion rate for a given context injection strategy.
    The task completion rate is determined based on the strategy's effectiveness.
    """
    ...
    base_rate = condition_effectiveness.get(condition_name, 0)
    
    # Introduce some variability to the task completion rate
    variability = np.random.normal(0, 0.05)  # mean 0, std 0.05
    return max(0, min(1, base_rate + variability))  # Clamp between 0 and 1
...
def run_experiment(condition_name, seed, harness):
    ...
    if np.isnan(task_completion_rate) or task_completion_rate > 100:
        print(f'FAIL: NaN/divergence detected in {condition_name} seed={seed}')

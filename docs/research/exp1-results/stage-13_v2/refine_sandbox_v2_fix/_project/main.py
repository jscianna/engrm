...
def simulate_task_completion_rate(condition_name):
    """
    Simulate the task completion rate based on the condition and context injection strategies.
    """
    # Define baseline task completion rates for different strategies
    ...
    # Fetch the base performance for the strategy
    base_rate = condition_performance.get(condition_name, None)
    
    if base_rate is None:
        raise ValueError(f"Unrecognized condition: {condition_name}")
    
    # Simulate variability in task completion rates using a normal distribution
    simulated_rate = np.random.normal(loc=base_rate, scale=0.05)
    
    # Ensure simulated rate stays within valid bounds
    simulated_rate = max(0.0, min(1.0, simulated_rate))
...
def run_experiment(condition_name, seed, harness):
    ...
    try:
        task_completion_rate = simulate_task_completion_rate(condition_name)

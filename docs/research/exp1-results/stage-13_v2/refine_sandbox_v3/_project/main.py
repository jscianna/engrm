...
def simulate_task_completion(condition_name):
    """
    Simulates task completion rate based on condition.
    Implements a more sophisticated model considering multiple factors.
    """
    # Base performance values for each strategy
    base_performance = {
    ...
    # Simulate adjustments based on context prioritization and user behavior
    context_importance_factor = np.random.uniform(0.9, 1.1)
    user_behavior_factor = np.random.uniform(0.95, 1.05)
    historical_data_factor = np.random.uniform(0.85, 1.15)

    task_completion_rate = (
        base_performance.get(condition_name, 0.5) *
        context_importance_factor *
        user_behavior_factor *
        historical_data_factor
    )
...
def run_experiment(condition_name, seed, harness):
    ...
    task_completion_rate = simulate_task_completion(condition_name)

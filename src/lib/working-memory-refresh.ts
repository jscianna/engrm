 ...
 */
export async function refreshWorkingMemory(
  userId: string
): Promise<WorkingMemoryRefreshStats> {
  await ensureInitialized();
  const client = getDb();

  const stats: WorkingMemoryRefreshStats = {
    promoted: 0,
    demoted: 0,
  };
 ...
 */
export async function previewWorkingMemoryRefresh(
  userId: string
): Promise<{
  promoted: { count: number; sampleIds: string[] };
  demoted: { count: number; sampleIds: string[] };
}> {
  await ensureInitialized();
  const client = getDb();
  ...
  return {
    promoted: {
      count: Number((promoteCountResult.rows[0] as { count?: number })?.count ?? 0),
      sampleIds: promoteResult.rows.map((row) => row.id as string),
    },
    demoted: {
      count: Number((demoteCountResult.rows[0] as { count?: number })?.count ?? 0),
      sampleIds: demoteResult.rows.map((row) => row.id as string),
    },
  };
}

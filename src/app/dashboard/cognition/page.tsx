...
import { Brain, CheckCircle2, Download, GitBranchPlus, ShieldAlert, Sparkles, Trash2 } from "lucide-react";
...
function cleanTraceDisplay(problem: string): string {
  let cleaned = problem
    // Strip System: prefix with optional timestamp
    .replace(/^System:\s*(\[\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}(?::\d{2})?\s+GMT[+-]\d+\]\s*)?/i, "")
    // Strip OpenClaw metadata blocks
    .replace(/OpenClaw runtime context \(internal\):[\s\S]*?(?=\n\n|\n[A-Z]|$)/gi, "")
    .replace(/\[Internal task completion event\][\s\S]*?(?=\n\n|$)/gi, "")
    // Strip session keys and IDs
    .replace(/(?:session_key|session_id|source|type):\s*\S+/gi, "")
    // Strip leading "task:" label
    .replace(/^task:\s*/i, "")
    // Clean up whitespace
    .replace(/\s+/g, " ")
    .trim();
...
export default async function CognitiveDashboardPage({
  ...
  const [metrics, settings, traces, patterns, skills, applications] = await Promise.all([
    ...
    <div className="space-y-6">
      <section className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-6">
        ...
        <p className="mt-2 max-w-3xl text-sm text-zinc-400">
          Monitor trace capture, pattern promotion, skill freshness, and application feedback from one place.
        </p>
      </section>
          ...
          <CardContent className="space-y-4">
            <div className="rounded-xl border border-zinc-800 bg-zinc-950/50 p-4">
              ...
              <Button asChild size="sm" className="mt-3">
                <a href="/dashboard/cognition/export">
              ...
              return (
                <div key={trace.id} className="rounded-xl border border-zinc-800 bg-zinc-950/50 p-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-sm font-medium text-zinc-100">{cleanTraceDisplay(trace.problem)}</p>
          ...
          <CardContent className="space-y-3">
            {skills.slice(0, 12).map((skill) => (
              <div key={skill.id} className="rounded-xl border border-zinc-800 bg-zinc-950/50 p-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <Link href={`/dashboard/cognition/skills/${skill.id}`} className="text-sm font-medium text-zinc-100 hover:text-cyan-300 transition-colors">{skill.name}</Link>

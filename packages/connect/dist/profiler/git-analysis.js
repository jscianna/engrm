...
export async function analyzeGitHistory(workspaceRoot) {
    const emptyResult = {
        totalCommits: 0,
    ...
    const hotspotStr = git(workspaceRoot, "log --format= --name-only --diff-filter=ACMR -n 500 HEAD");
...
export function getHeadCommit(workspaceRoot) {
    if (!existsSync(path.join(workspaceRoot, ".git")))
        return undefined;
    const sha = git(workspaceRoot, "rev-parse HEAD");
    return sha ?? undefined;
}
function computeHotspots(logOutput) {
    if (!logOutput)
        return [];
...
function computeActiveDirectories(logOutput) {
    if (!logOutput)
        return [];
...
function detectBranchingModel(workspaceRoot) {
...
//# sourceMappingURL=git-analysis.js.map
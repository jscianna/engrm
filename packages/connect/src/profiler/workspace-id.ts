...
function getGitRemoteUrl(workspaceRoot: string): string | null {
...
function formatHostedId(host: string, userRepo: string): string {
  if (host.includes("github")) return `gh:${userRepo}`;
  if (host.includes("gitlab")) return `gl:${userRepo}`;
  if (host.includes("bitbucket")) return `bb:${userRepo}`;

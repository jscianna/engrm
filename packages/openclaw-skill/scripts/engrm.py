...
def get_config():
        ...
        for line in secrets_path.read_text().splitlines():
            ...
            if "=" in line:
                ...
                if key == "ENGRM_API_KEY" and not api_key:
                    api_key = value
                elif key == "ENGRM_API_URL" and api_url == "https://engrm.xyz":
                    api_url = value
    ...
    if not api_key:
        print("Error: ENGRM_API_KEY not set.", file=sys.stderr)
        print("Set via environment or ~/.openclaw/secrets/engrm.env", file=sys.stderr)
        sys.exit(1)
    
    if not vault_password:
        print("Error: ENGRM_VAULT_PASSWORD not set.", file=sys.stderr)
...
def main():
    parser = argparse.ArgumentParser(
        description="Engrm Zero-Knowledge CLI - Your data never leaves your device unencrypted",

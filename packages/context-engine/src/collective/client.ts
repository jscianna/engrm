...
import type { CollectivePattern, SharedSignal, CollectiveApiConfig } from "./types.js";
 ...
 */
export async function submitSharedSignal(
  signal: SharedSignal,
  apiKey: string,
  baseUrl: string,
): Promise<void> {
...
export async function fetchCollectivePatterns(
  ...
  try {
    ...
    const response = await fetch(
      `${baseUrl}/v1/collective/query?${params.toString()}`,
      {
        method: "GET",
        headers: {
          "Authorization": `Bearer ${apiKey}`,
    ...
    const data = await response.json() as { patterns?: CollectivePattern[] };

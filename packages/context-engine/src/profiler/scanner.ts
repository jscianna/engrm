...
export async function scanFileTree(
  ...
  const languageLOC = new Map<string, number>();
  const filesByExtension = new Map<string, string[]>();
  const largestFiles: Array<{ path: string; loc: number }> = [];
...
function detectStructureType(workspaceRoot: string): "monorepo" | "single" | "workspace" {
  ...
  if (
    existsSync(path.join(workspaceRoot, "packages")) ||
...
async function detectWorkspaces(workspaceRoot: string): Promise<string[]> {
...
function detectEntryPoints(workspaceRoot: string, allFiles: string[]): string[] {
...
async function buildFileTreeSummary(
  workspaceRoot: string,
  maxDepth: number,
  ...
  async function walk(dir: string, depth: number, prefix: string): Promise<void> {
    ...
    if (depth <= 1 && files.length > 0) {
      const configFiles = files.filter((f) =>
        /\.(json|ya?ml|toml|lock|config\.\w+)$/.test(f.name) ||
        f.name.startsWith(".")
      );

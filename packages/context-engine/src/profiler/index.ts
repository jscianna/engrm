...
import { formatCodebaseProfileForInjection } from "./serializer.js";
...
export async function profileCodebase(
  ...
  const [scanResult, techStack, gitResult, headCommit] = await Promise.all([
    ...
    analyzeGitHistory(workspaceRoot).catch((): Awaited<ReturnType<typeof analyzeGitHistory>> => ({
      totalCommits: 0,
  ...
  const importResult = await analyzeImports(workspaceRoot, scanResult.allFiles).catch(
    (): Awaited<ReturnType<typeof analyzeImports>> => ({
      topImports: [],
      mostImportedFiles: [],
    }),
  );
  ...
  // Build hotspots
  const hotspots: CodebaseProfile["hotspots"] = [];
  ...
  const peerProjects = importResult.topImports
    .filter((imp) => imp.module.startsWith("@") && scanResult.workspaces.some(
      (ws) => imp.module.includes(path.basename(ws)),
    ))
    .map((imp) => imp.module);
 ...
 */
export async function loadCodebaseProfile(workspaceRoot: string): Promise<CodebaseProfile | null> {
  const profilePath = getProfilePath(workspaceRoot);
  if (!existsSync(profilePath)) return null;

  try {
    const content = await readFile(profilePath, "utf-8");
    const profile = JSON.parse(content) as CodebaseProfile;
...
function getProfilePath(workspaceRoot: string): string {

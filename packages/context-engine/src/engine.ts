...
import {
  ...
  analyzeSession,
  mergeSignals,
  formatUserDNAForInjection,
  loadUserDNA,
  saveUserDNA,
  type UserDNA,
} from "./user-dna/index.js";
import {
  processTraceForCollective,
  getCollectiveWisdom,
  type TraceData,
...
export class FatHippoContextEngine implements ContextEngine {
  ...
  private runtimeClient: FatHippoHostedRuntimeClientContract | null;
  private config: FatHippoConfig;
  private mode: RuntimeMode;
  private localStore: LocalMemoryStore | null;
  private hostedSessions: Map<string, { hostedSessionId: string; workspaceRoot?: string }> = new Map();
  ...
  // User DNA state
  private userDNACache: Map<string, UserDNA | null> = new Map();
  private userDNAEnabled: boolean;
  ...
  async assemble(params: {
    ...
    let hostedContext = "";
    let cue: HippoHelpCue | null = null;
    ...
    try {
      ...
      if (hostedContext.includes("## Recommended Workflow")) {
        ...
        hostedContext.includes("## Learned Coding Patterns") ||
    ...
    const userDNABlock = await this.getUserDNABlock(
      hostedSession.workspaceRoot,
    );
  ...
  async onSubagentEnded(params: {
    ...
    await this.endHostedSession(
      params.childSessionKey,
      this.mapHostedOutcomeFromReason(params.reason),
    );
  }
  ...
  async dispose(): Promise<void> {
    await Promise.allSettled(
      [...this.hostedSessions.keys()].map((sessionId) =>
        this.endHostedSession(sessionId, "abandoned"),
      ),
    );
  ...
  private async updateUserDNA(params: {
    sessionId: string;
    sessionFile: string;
    messages: AgentMessage[];
  }): Promise<void> {
    if (!this.userDNAEnabled) return;
    ...
    try {
      ...
      if (runtimeMessages.length < 2) return;
  ...
  private async getUserDNABlock(workspaceRoot?: string): Promise<string> {
    if (!this.userDNAEnabled) return "";
    ...
    try {
      ...
      if (!dna || dna.sessionCount === 0) return "";
  ...
  private async processTraceForCollectiveSharing(params: {
    sessionId: string;
    sessionFile: string;
    messages: AgentMessage[];
  }): Promise<void> {
    if (!this.collectiveEnabled || !this.config.apiKey || !this.config.baseUrl) return;

    try {
      if (!shouldCaptureCodingTrace(params.messages)) return;
      ...
      processTraceForCollective(
        traceData,
        { sharedLearningEnabled: this.config.shareEligibleByDefault !== false },
        { apiKey: this.config.apiKey, baseUrl: this.config.baseUrl || "https://fathippo.ai/api" },
      ).catch((error) => {
  ...
  private async getCollectiveWisdomBlock(
    lastUserMessage: string,
    messages: AgentMessage[],
  ): Promise<string> {
    if (!this.collectiveEnabled || !this.config.apiKey || !this.config.baseUrl) return "";
    ...
    try {
      ...
      let framework = "";
      if (/next\.?js|app\s+router/i.test(combined)) framework = "next.js";
      else if (/react|useState|useEffect/i.test(combined)) framework = "react";
      else if (/express/i.test(combined)) framework = "express";
      else if (/turso|libsql/i.test(combined)) framework = "sqlite";
      else if (/typescript|tsc/i.test(combined)) framework = "typescript";

      return await getCollectiveWisdom(
        errorType,
        framework,
        { apiKey: this.config.apiKey, baseUrl: this.config.baseUrl || "https://fathippo.ai/api" },
      );
  ...
  private buildHostedRuntimeMetadata(params?: {
    sessionId?: string;
    sessionFile?: string;
  ...
  private toRuntimeMessages(messages: AgentMessage[]): FatHippoConversationMessage[] {
    return messages
      .map((message): FatHippoConversationMessage | null => {
        ...
        const rawRole =
          typeof raw.role === "string"
            ...
            : typeof raw.type === "string"
              ...
              : "assistant";
        const role: FatHippoConversationMessage["role"] =
          rawRole === "system"
            ? "system"
            : rawRole === "user"
              ? "user"
      ...
      .filter((message): message is FatHippoConversationMessage => message !== null);
  }
  ...
  private extractTurnMessages(
    ...
    const sliced =
      Number.isFinite(prePromptMessageCount) && prePromptMessageCount >= 0
  ...
  private async captureLocalTurnMemories(params: {
    sessionId: string;
    sessionFile: string;
  ...
  private async endHostedSession(
    sessionId: string,

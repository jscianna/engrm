...
export function detectTaskType(
  ...
  const scores: Record<Exclude<TaskType, 'general'>, number> = {
    debugging: 0,
    building: 0,
    refactoring: 0,
    reviewing: 0,
  ...
  const matchedSignals: Record<Exclude<TaskType, 'general'>, string[]> = {
    debugging: [],
    building: [],
    refactoring: [],
    reviewing: [],
  ...
  for (const msg of recent) {
    ...
    for (const [taskType, patterns] of Object.entries(TASK_PATTERNS) as Array<
      [Exclude<TaskType, 'general'>, RegExp[]]
    >) {
  ...
  for (const [taskType, score] of Object.entries(scores) as Array<
    [Exclude<TaskType, 'general'>, number]
  >) {
    if (score > bestScore) {
  ...
  return {
    ...
    signals: matchedSignals[bestType as Exclude<TaskType, 'general'>] ?? [],
  };
}

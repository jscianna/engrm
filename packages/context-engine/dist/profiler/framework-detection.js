...
export async function detectTechStack(workspaceRoot) {
...
function matchesRule(workspaceRoot, rule, allDeps) {
    ...
    if (rule.files) {
        for (const file of rule.files) {
            ...
            if (existsSync(path.join(workspaceRoot, file)))
                return true;
        }
    }
    return false;
}
async function gatherAllDependencies(workspaceRoot) {
    const deps = new Set();
    // package.json (JS/TS)
...
async function parsePackageJsonDeps(filePath, deps) {
...
async function parsePythonDeps(filePath, deps) {
    if (!existsSync(filePath))
        return;
    try {
        const content = await readFile(filePath, "utf-8");
...
async function parsePyprojectDeps(filePath, deps) {
    if (!existsSync(filePath))
        return;
    try {
        const content = await readFile(filePath, "utf-8");
...
async function parseCargoDeps(filePath, deps) {
    if (!existsSync(filePath))
        return;
    try {
        const content = await readFile(filePath, "utf-8");
...
async function parseGoModDeps(filePath, deps) {
    if (!existsSync(filePath))
        return;
    try {
        const content = await readFile(filePath, "utf-8");
...
async function parseGemfileDeps(filePath, deps) {
    if (!existsSync(filePath))
        return;
    try {
        const content = await readFile(filePath, "utf-8");
...
function detectPackageManager(workspaceRoot) {
    if (existsSync(path.join(workspaceRoot, "pnpm-lock.yaml")))
        return "pnpm";
    if (existsSync(path.join(workspaceRoot, "bun.lockb")) || existsSync(path.join(workspaceRoot, "bun.lock")))
        return "bun";
    ...
    if (existsSync(path.join(workspaceRoot, "go.mod")))
        return "go";
    if (existsSync(path.join(workspaceRoot, "Pipfile")))
        return "pipenv";
    if (existsSync(path.join(workspaceRoot, "poetry.lock")))
        return "poetry";
    if (existsSync(path.join(workspaceRoot, "requirements.txt")))
        return "pip";
    if (existsSync(path.join(workspaceRoot, "Gemfile")))
        return "bundler";
    if (existsSync(path.join(workspaceRoot, "deno.json")) || existsSync(path.join(workspaceRoot, "deno.lock")))
        return "deno";
    return "unknown";
}
async function detectRuntime(workspaceRoot) {
    ...
    if (existsSync(pkgPath))
        return "node";
    if (existsSync(path.join(workspaceRoot, "requirements.txt")) || existsSync(path.join(workspaceRoot, "pyproject.toml")))
        return "python";
    if (existsSync(goMod))
        return "go";
    if (existsSync(cargoToml))

...
export function mergeSignals(existing, newSignals) {
    ...
    const communication = {
        verbosity: existing.confidence.communication > 0.1
            ? emaCategory(existing.communication.verbosity, newVerbosity, alpha)
            : newVerbosity,
        style: existing.confidence.communication > 0.1
            ? emaCategory(existing.communication.style, newStyle, alpha)
            : newStyle,
        preferredResponseLength: existing.confidence.communication > 0.1
            ? emaCategory(existing.communication.preferredResponseLength, newResponseLength, alpha)
            : newResponseLength,
    };
    // Work patterns
    ...
    const workPatterns = {
        ...
        sessionType: classifySessionType(avgMinutes),
    ...
    const newNamingStyle = totalNaming === 0
    ...
    const conventions = {
        namingStyle: existing.confidence.conventions > 0.1
            ? emaCategory(existing.conventions.namingStyle, newNamingStyle, alpha)
            : newNamingStyle,
 ...
 */
export function generateDirectives(dna) {
    const directives = [];
    // Communication style
...
// --- Internal helpers ---
function ema(current, incoming, alpha) {
    return Math.round((alpha * incoming + (1 - alpha) * current) * 1000) / 1000;
}
function emaCategory(current, incoming, alpha) {
...
function classifyVerbosity(avgLength) {
    if (avgLength < 60)
        return "terse";
    if (avgLength > 200)
        return "detailed";
...
function classifyStyle(questionRatio, commandRatio) {
    if (commandRatio > 0.6)
        return "commands";
...
function classifyResponseLength(avgLength) {
    if (avgLength < 40)
        return "short";
    if (avgLength > 150)
        return "long";
...
function classifySessionType(avgMinutes) {
    if (avgMinutes < 10)
        return "quick-fixes";
    if (avgMinutes > 30)
        return "deep-sessions";
    return "mixed";
}
function computeKeywordSignal(text, keywords) {
    const matches = keywords.filter((kw) => text.includes(kw)).length;
    // Normalize: 0 matches = 0, 3+ matches = ~1.0
    return Math.min(1, matches / 3);
}
function detectFocusAreas(files, text) {
...
function detectNamingConventions(text, files) {
...
function detectPreferredPatterns(text, files) {
    const detected = [];
    const combined = text + " " + files.join(" ");
...
function detectAvoidPatterns(userMessages) {
    const avoidPhrases = [];
    for (const msg of userMessages) {
...
function mergeFocusAreas(existing, incoming) {
...
function mergeStringArrayWeighted(existing, incoming, maxSize) {

...
function recallAtK(ids: string[], expected: string[]): number {
  if (expected.length === 0) {
    return 0;
  }
...
function hitRate(ids: string[], expected: string[]): number {
  if (expected.length === 0) {
    return 0;
  }
...
function weakOutcomeScore(
  ...
  const matchedExpected =
    fixture.expectedPatternIds.some((id) => prediction.patterns.some((pattern) => pattern.id === id)) ||
...
function timeToResolutionDelta(
  fixture: RetrievalEvalFixture,
  prediction: RetrievalEvalPrediction,
): number {
  if (
    fixture.baseline?.medianTimeToResolutionMs == null ||
...
export function evaluateBenchmarkGate(params: {
  ...
  if (
    thresholds.minPatternRecallAtK != null &&
    params.current.patternRecallAtK < thresholds.minPatternRecallAtK
  ) {
    reasons.push(
      `pattern recall@k ${params.current.patternRecallAtK.toFixed(3)} below minimum ${thresholds.minPatternRecallAtK.toFixed(3)}`,
    );
  }
  ...
  if (
    thresholds.minWeakOutcomeLift != null &&
    params.current.weakOutcomeLift < thresholds.minWeakOutcomeLift
  ) {
    reasons.push(
      `outcome lift ${params.current.weakOutcomeLift.toFixed(3)} below minimum ${thresholds.minWeakOutcomeLift.toFixed(3)}`,
    );
  }
  ...
  if (
    thresholds.minVerificationCompletionRate != null &&
    params.current.verificationCompletionRate < thresholds.minVerificationCompletionRate
  ) {
    reasons.push(
      `verification completion ${params.current.verificationCompletionRate.toFixed(3)} below minimum ${thresholds.minVerificationCompletionRate.toFixed(3)}`,
    );
  }

  if (params.baseline) {
    if (
      thresholds.maxTraceMrrRegression != null &&
    ...
    if (
      thresholds.maxPatternRecallAtKRegression != null &&
    ...
    if (
      thresholds.maxSkillHitRateRegression != null &&
    ...
    if (
      thresholds.maxWeakOutcomeLiftRegression != null &&
    ...
    if (
      thresholds.maxSuccessRateRegression != null &&
    ...
    if (
      thresholds.maxVerificationCompletionRateRegression != null &&
    ...
    if (
      thresholds.maxRetryDeltaRegression != null &&
    ...
    if (
      thresholds.maxTimeToResolutionDeltaRegressionMs != null &&

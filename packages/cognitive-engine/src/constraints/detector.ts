...
export interface Constraint {
  ...
  source: string;  // Original message that created this
  active: boolean;
  createdAt: string;
  updatedAt: string;
}
 ...
 */
export function detectConstraint(message: string): {
 ...
 */
function determineSeverity(message: string): ConstraintSeverity {
  const criticalWords = [
 ...
 */
export function checkActionAgainstConstraints(
  action: string,
  constraints: Constraint[]
): { violated: Constraint[]; warnings: Constraint[] } {
  ...
  for (const constraint of constraints) {
    ...
    const matches = constraint.triggers.some(trigger => 
 ...
 */
export function formatConstraintsForInjection(constraints: Constraint[]): string {

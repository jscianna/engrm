...
export function detectModelFamily(model) {
    ...
    if (SMALL_MODEL_PATTERNS.test(m))
        return "small";
    if (CLAUDE_PATTERNS.test(m))
        return "claude";
    if (GPT_PATTERNS.test(m))
        return "gpt";
    if (DEEPSEEK_PATTERNS.test(m))
        return "deepseek";
    if (GEMINI_PATTERNS.test(m))
        return "gemini";
    return "unknown";
}
// --- Context Budgets ---
...
export function getContextBudget(family) {
...
// --- Budget Enforcement ---
function enforce_budget(context, budget) {
...
function enforce_token_budget(formatted, budget) {
...
function format_claude(context) {
    const sections = [];
    if (context.critical?.length) {
        const items = context.critical.map((m) => `  <memory title="${escape_xml(m.title)}">${escape_xml(m.text)}</memory>`);
        sections.push(`<critical>\n${items.join("\n")}\n</critical>`);
    }
    if (context.working?.length) {
        const items = context.working.map((m) => `  <memory title="${escape_xml(m.title)}">${escape_xml(m.text)}</memory>`);
        sections.push(`<working>\n${items.join("\n")}\n</working>`);
    }
    if (context.high?.length) {
        const items = context.high.map((m) => `  <memory title="${escape_xml(m.title)}">${escape_xml(m.text)}</memory>`);
...
function format_deepseek(context) {
    const all_memories = [
        ...(context.critical ?? []),
        ...(context.working ?? []),
        ...(context.high ?? []),
...
function format_default(context) {
    const sections = [];
    if (context.critical?.length) {
 ...
 */
export function formatTieredContextForModel(context, family) {
 ...
 */
export function formatMemoryListForModel(memories, family) {
    if (family === "small") {
        return memories.map((m) => truncate(`${m.title}: ${m.text}`, 60));
    }
    if (family === "claude" || family === "gemini") {
        return memories.map((m) => `<memory title="${escape_xml(m.title)}">${escape_xml(m.text)}</memory>`);
    }
    if (family === "gpt") {

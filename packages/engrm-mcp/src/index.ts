...
async function handleDelete(params: { id: string }): Promise<string> {
...
export async function createServer(): Promise<Server> {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      switch (name) {
        case "memry_store":
          result = await handleStore(args as Parameters<typeof handleStore>[0]);
          break;
        case "memry_search":
          result = await handleSearch(args as Parameters<typeof handleSearch>[0]);
          break;
        case "memry_context":
          result = await handleContext(args as Parameters<typeof handleContext>[0]);
          break;
        case "memry_list":
          result = await handleList(args as Parameters<typeof handleList>[0]);
          break;
        case "memry_delete":
          result = await handleDelete(args as Parameters<typeof handleDelete>[0]);
          break;
        default:

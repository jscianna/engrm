...
export type LocalWorkflowStrategyKey =
  | "verify_first"
...
type LocalProfile = {
  memories: LocalStoredMemory[];
  traces: LocalStoredTrace[];
};
...
export interface LocalMemoryStore {
  remember(params: { profileId: string; content: string; title?: string }): Promise<LocalStoredMemory>;
  search(params: { profileId: string; query: string; limit?: number }): Promise<LocalMemorySearchResult[]>;
  getCriticalMemories(params: { profileId: string; limit?: number }): Promise<LocalStoredMemory[]>;
  getIndexedSummaries(params: { profileId: string; limit?: number }): Promise<LocalIndexedMemoriesResponse>;
  ...
  getCognitiveContext(params: { profileId: string; problem: string; limit?: number }): Promise<LocalCognitiveContext>;
}

const DEFAULT_MAX_MEMORIES = 500;
const DEFAULT_STORAGE_PATH = path.join(os.homedir(), ".openclaw", "fathippo-local.json");

const WORKFLOW_DEFINITIONS: Record<LocalWorkflowStrategyKey, { title: string; steps: string[] }> = {
  verify_first: {
...
function deriveImportanceTier(accessCount: number): LocalMemoryImportanceTier {
  if (accessCount >= 12) {
    return "critical";
  }
  if (accessCount >= 5) {
...
function detectWorkflow(params: LocalTraceInput): LocalWorkflowStrategyKey {
  ...
  const configHeavy =
    /\bconfig|middleware|auth|env|tsconfig|package\.json|next\.config|eslint|docker|prisma|drizzle\b/.test(problem) ||
    filesModified.some((file) =>
      /(config|middleware|auth|package\.json|tsconfig|next\.config|eslint|docker|prisma|drizzle)/i.test(file),
    );
...
function traceKeywords(trace: Pick<LocalStoredTrace, "problem" | "technologies" | "errorMessages" | "type">): string[] {
  return [
...
function computeTraceSimilarity(problem: string, trace: LocalStoredTrace): number {
  const base = jaccard(tokenize(problem), traceKeywords(trace));
  const substringBonus =
    normalizeText(trace.problem).includes(normalizeText(problem)) ||
...
function scoreTraceOutcome(trace: LocalStoredTrace): number {
...
function buildPatternTitle(trace: LocalStoredTrace): string {
...
function summarizeApproach(trace: LocalStoredTrace): string {
  const source = trace.solution || trace.reasoning || trace.problem;
  const normalized = source.trim().replace(/\s+/g, " ");
  return normalized.length > 180 ? `${normalized.slice(0, 177)}...` : normalized;
}

async function ensureParentDir(storagePath: string): Promise<void> {
  await mkdir(path.dirname(storagePath), { recursive: true });
}

export function createLocalMemoryStore(options: LocalMemoryStoreOptions = {}): LocalMemoryStore {
  ...
  let statePromise: Promise<LocalStoreFile> | null = null;

  async function loadState(): Promise<LocalStoreFile> {
    if (!statePromise) {
      statePromise = (async () => {
        try {
          ...
          const parsed = JSON.parse(raw) as {
            version?: number;
            profiles?: Record<string, Partial<LocalProfile>>;
          };
          if (
            parsed &&
            ...
            const profiles = Object.fromEntries(
              Object.entries(parsed.profiles).map(([profileId, profile]) => [
                profileId,
                {
                  memories: Array.isArray(profile?.memories) ? (profile.memories as LocalStoredMemory[]) : [],
                  traces: Array.isArray(profile?.traces) ? (profile.traces as LocalStoredTrace[]) : [],
                },
              ]),
            );
  ...
  async function saveState(nextState: LocalStoreFile): Promise<void> {
  ...
  function getProfile(state: LocalStoreFile, profileId: string): LocalProfile {
  ...
  function trimProfile(profile: LocalProfile): void {
  ...
  return {
    async remember(params) {
      ...
      const created: LocalStoredMemory = {
    ...
    async getIndexedSummaries(params) {
      ...
      const indices: LocalIndexedMemory[] = selected.map((memory) => ({
        index: memory.id,
    ...
    async getMemoriesByIds(profileId, ids) {
      ...
      return ids
        ...
        .filter((memory): memory is LocalStoredMemory => memory != null);
    },
    ...
    async learnTrace(params) {
      ...
      const existing = profile.traces.find(
        (trace) =>
          trace.type === params.type && normalizeText(trace.problem) === normalizedProblem,
      );
      const nextTrace: LocalStoredTrace = existing ?? {
    ...
    async getCognitiveContext(params) {
      ...
      const workflowStats = new Map<LocalWorkflowStrategyKey, { count: number; scoreSum: number }>();
      ...
      let workflow: LocalWorkflowRecommendation | null = null;
      ...
      const patterns: LocalPatternRecommendation[] = matches

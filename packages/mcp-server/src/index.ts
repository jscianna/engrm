...
const TOOLS: Tool[] = [
  ...
  },
  {
    name: "recall",
    description:
      "Get relevant context from FatHippo for a given message. Returns memories that are semantically related to the input.",
    inputSchema: {
      type: "object" as const,
      properties: {
        message: {
  ...
  },
  {
    name: "search",
    description:
      "Search memories in FatHippo. Returns memories matching the query.",
    inputSchema: {
      type: "object" as const,
      properties: {
        query: {
...
async function handleRemember(args: {
  ...
  const result = await apiRequest<{ id: string }>("/v1/simple/remember", {
    text: args.text,
    title: args.title,
  });
...
async function handleRecall(args: { message: string }): Promise<string> {
...
async function handleSearch(args: {
  ...
  const result = await apiRequest<{
    results: Array<{ memory: { title: string; content: string }; score: number }>;
  }>("/v1/search", {
    query: args.query,
    limit: args.limit || 10,
  });
  ...
  const formatted = result.results
    .map(
      (r, i) =>
        `${i + 1}. [${Math.round(r.score * 100)}%] ${r.memory.title || "Memory"}\n   ${r.memory.content}`
    )
    .join("\n\n");
...
async function main() {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      switch (name) {
        case "remember":
          result = await handleRemember(args as { text: string; title?: string });
          break;
        case "recall":
          result = await handleRecall(args as { message: string });
          break;
        case "search":
          result = await handleSearch(args as { query: string; limit?: number });
          break;
        default:
